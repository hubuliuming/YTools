...
{
    public class MenuOptions : UnityEditor.Editor
    {
        ...
        private static Transform GetCanvasTrans()
        {
            ...
            if (canvas == null)
            {
                Type [] types = new Type[] { typeof(Canvas),typeof(CanvasScaler),
                    typeof(GraphicRaycaster),typeof(RectTransform)};
                var canvasGo = new GameObject("Canvas",types);
                SetLayer(canvasGo);
        ...
        private static GameObject CreatePolygonColliderImage()
        {
